...
const AuthHeader = ({AuthMode = "signIn"}) => {
	...
	const headerLogo = {
		signIn: <Image source={require("../../assets/Images/auth_logo.png")} className="w-2/4 h-20"
					   resizeMode={`contain`}/>,
		signUp: (
	...
	return (
		...
		>
			...
			<Text className="text-4xl text-white font-bold mt-8">{headerText[AuthMode].main}</Text>

			{headerText[AuthMode].sub && (
				<Text className="text-4xl text-white font-bold mt-2">{headerText[AuthMode].sub}</Text>
			)}

			<View className="flex flex-row justify-start gap-1 mt-4 items-center">
				{footerText[AuthMode]}
			</View>
		</ImageBackground>
	);
